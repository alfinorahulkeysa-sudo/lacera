            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="5" class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:outline-none focus:border-[#e81c62] focus:ring-1 focus:ring-[#e81c62] text-sm">{{ old('description', $product->description) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                ...
                <div>
                    ...
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-xl hover:bg-gray-50 transition cursor-pointer relative">
                        ...
                        {{-- Jika produk sudah punya gambar, hilangkan class 'hidden' di awal --}}
                        <div id="preview-container" class="{{ $product->image ? '' : 'hidden' }} absolute inset-0 bg-white rounded-xl flex items-center justify-center p-2 z-10">
                            <img id="image-preview" src="{{ $product->image ? asset('storage/' . $product->image) : '#' }}" alt="Preview" class="max-h-full max-w-full object-contain rounded-lg">
                            <button type="button" id="remove-preview" class="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1.5 hover:bg-red-600 shadow-md text-xs transition flex items-center gap-1">
                                <i class="fas fa-times"></i> Hapus / Ganti
                            </button>
                        </div>
