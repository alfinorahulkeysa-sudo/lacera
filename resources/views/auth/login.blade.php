...
@section('title', 'Masuk Akun - Lacera Official Store')
...
<div class="bg-gray-50 min-h-screen">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <nav class="flex text-xs text-gray-500 font-medium">
            ...
            <span class="mx-2 text-gray-300">&gt;</span>
            <span class="text-pink-600 font-bold">Masuk</span>
        </nav>
    </div>

    {{-- Title --}}
    <div class="text-center mb-8 px-4">
        <h1 class="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight">Masuk ke Akun Anda</h1>
        <p class="text-sm text-gray-500 mt-2">Selamat datang kembali! Silakan masuk untuk melanjutkan pengalaman belanja Anda.</p>
    </div>
            ...
            <div class="lg:col-span-7">
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
                    <div class="flex items-center justify-between mb-6">
                        <div>
                            <h2 class="text-lg font-bold text-gray-900">Form Login Customer</h2>
                            <p class="text-xs text-gray-500 mt-1">Masukkan email dan kata sandi yang telah terdaftar.</p>
                        </div>
                    ...
                    {{-- Session Status --}}
                    @if(session('status'))
                        <div class="mb-4 text-xs font-semibold text-green-600 bg-green-50 border border-green-200 rounded-lg p-3">{{ session('status') }}</div>
                    @endif
                    ...
                    <form method="POST" action="{{ route('login') }}" class="space-y-4">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1.5">Email atau Nomor Telepon</label>
                            <div class="relative">
                        ...
                        <div>
                            ...
                            <div class="relative" x-data="{ show: false }">
                                ...
                                <button type="button" @click="show = !show" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-gray-600">
                                    ...
                                    <svg x-show="show" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.542-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L3 3m6.878 6.878l4.242 4.242M15.12 15.12L21 21"/></svg>
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            <label class="inline-flex items-center cursor-pointer select-none">
                                <input type="checkbox" name="remember" class="rounded border-gray-300 text-pink-600 focus:ring-pink-500 w-4 h-4">
                                <span class="ml-2 text-xs text-gray-600 font-medium">Ingat saya</span>
                            </label>
                            @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-xs text-pink-600 font-bold hover:underline">Lupa kata sandi?</a>
                            @endif
                        </div>
                    ...
                    <div class="mt-6 pt-4 border-t border-gray-100 text-center">
                        <p class="text-xs text-gray-500">Belum punya akun? <a href="{{ route('register') }}" class="text-pink-600 font-bold hover:underline">Daftar sekarang di sini</a></p>
                    </div>
                </div>
            </div>
